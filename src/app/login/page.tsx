...
export default function LoginPage() {
  ...
  const handleSignIn = async () => {
    try {
      ...
      // Not pushing to router here to allow the redirect or the useEffect to handle it
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center overflow-hidden relative">
      ...
      <div className="relative z-10 sm:mx-auto sm:w-full sm:max-w-md">
        <motion.div 
          ...
          className="glass-card mx-4 sm:mx-0 p-8 py-12 rounded-3xl border border-white/10 shadow-2xl shadow-indigo-500/10 text-center"
        >
          <div className="inline-flex bg-indigo-500/20 p-4 rounded-2xl mb-6">
          ...
          <p className="mt-2 text-sm text-slate-400 mb-8">
            Sign in to access your AI action plans and beat your deadlines.
          </p>
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 bg-white hover:bg-slate-200 text-slate-950 px-4 py-4 rounded-xl font-bold transition-all disabled:opacity-70 disabled:cursor-not-allowed hover:scale-[1.02] active:scale-[0.98]"
          >
            {loading ? (
